...
@implementation QCloudCreateDirectoryRequest
- (void) dealloc
{
}
-  (instancetype) init
...
- (void) configureReuqestSerializer:(QCloudRequestSerializer *)requestSerializer  responseSerializer:(QCloudResponseSerializer *)responseSerializer
{
    ...
    NSArray* responseSerializers = @[
                                    ...
                                    QCloudResponseObjectSerilizerBlock([QCloudCreateDirectoryResult class])
                                    ];
...
- (BOOL) buildRequestData:(NSError *__autoreleasing *)error
{
    ...
    if (!self.directory || ([self.directory isKindOfClass:NSString.class] && ((NSString*)self.directory).length == 0)) {
        if (error != NULL) {
            *error = [NSError qcloud_errorWithCode:QCloudNetworkErrorCodeParamterInvalid message:[NSString stringWithFormat:@"paramter[directory] is invalid (nil), it must have some value. please check it"]];
            return NO;
        }
    }
      [self.requestData setQueryStringParamter:@"/" withKey:@"/"];
    [self.requestData setParameter:@"create" withKey:@"op"];
    ...
    [__pathComponents addObject:@"/"];
    self.requestData.URIComponents = __pathComponents;
    return YES;
}
- (void) setFinishBlock:(void (^)(QCloudCreateDirectoryResult* result, NSError * error))QCloudRequestFinishBlock
{
