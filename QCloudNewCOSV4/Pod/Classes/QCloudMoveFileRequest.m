...
@implementation QCloudMoveFileRequest
- (void) dealloc
{
}
-  (instancetype) init
...
- (BOOL) buildRequestData:(NSError *__autoreleasing *)error
{
    ...
    if (!self.destination || ([self.destination isKindOfClass:NSString.class] && ((NSString*)self.destination).length == 0)) {
        if (error != NULL) {
            *error = [NSError qcloud_errorWithCode:QCloudNetworkErrorCodeParamterInvalid message:[NSString stringWithFormat:@"paramter[destination] is invalid (nil), it must have some value. please check it"]];
            return NO;
        }
    }
    [self.requestData setParameter:self.destination withKey:@"dest_fileid"];
    [self.requestData setNumberParamter:@(self.overWrite) withKey:@"to_over_write"];
    ...
    [self.requestData setParameter:@"move" withKey:@"op"];
