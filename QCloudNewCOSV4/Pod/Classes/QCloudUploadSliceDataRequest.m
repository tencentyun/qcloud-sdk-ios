...
@implementation QCloudUploadSliceDataRequest
- (void) dealloc
{
}
-  (instancetype) init
...
- (void) configureReuqestSerializer:(QCloudRequestSerializer *)requestSerializer  responseSerializer:(QCloudResponseSerializer *)responseSerializer
{
    ...
    NSArray* responseSerializers = @[
                                    ...
                                    QCloudResponseObjectSerilizerBlock([QCloudUploadSliceResult class])
                                    ];
...
- (BOOL) buildRequestData:(NSError *__autoreleasing *)error
{
    ...
    [self.requestData setParameter:@"upload_slice_data" withKey:@"op"];
    ...
    if (!self.localPath || ([self.localPath isKindOfClass:NSString.class] && ((NSString*)self.localPath).length == 0)) {
        if (error != NULL) {
            *error = [NSError qcloud_errorWithCode:QCloudNetworkErrorCodeParamterInvalid message:[NSString stringWithFormat:@"paramter[localPath] is invalid (nil), it must have some value. please check it"]];
            return NO;
        }
    }
    [self.requestData setParameter:self.localPath withKey:@"localPath"];
    [self.requestData setParameter:self.session withKey:@"session"];
    [self.requestData setNumberParamter:@(self.offset) withKey:@"offset"];
    [self.requestData setNumberParamter:@(self.sliceLength) withKey:@"sliceLength"];
    [self.requestData setParameter:self.sha withKey:@"sha"];
    [self.requestData setNumberParamter:@(self.fileLength) withKey:@"file_size"];
...
- (void) setFinishBlock:(void (^)(QCloudUploadSliceResult* result, NSError * error))QCloudRequestFinishBlock
{
