...
@interface QCloudPNetInfoTool()
@property (nonatomic,copy) NSDictionary *ipInfoDict;
@property (nonatomic,copy) NSDictionary *wifiDict;
@end


@implementation QCloudPNetInfoTool


static QCloudPNetInfoTool *QCloudPNetInfoTool_instance = NULL;

+ (instancetype)shareInstance
{
    if (QCloudPNetInfoTool_instance == NULL) {
        QCloudPNetInfoTool_instance = [[QCloudPNetInfoTool alloc] init];
    }
    return QCloudPNetInfoTool_instance;
}

- (void)refreshNetInfo
{
    // net info
    self.ipInfoDict = [QCloudPNetInfoTool getIPAddresses];
...
{
    NSString *netType = @"";
    QCloudPNetReachability *reachNet = [QCloudPNetReachability reachabilityWithHostName:@"www.apple.com"];
