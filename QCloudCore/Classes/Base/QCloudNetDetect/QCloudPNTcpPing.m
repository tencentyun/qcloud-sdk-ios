...
@interface QCloudPNTcpPingResult()
...
@implementation QCloudPNTcpPingResult
...
{
    return [NSString stringWithFormat:@"TCP_conn_loss=%lu,min=%.2f,avg=%.2f,max=%.2fms",(unsigned long)self.loss,self.min_time,self.avg_time,self.max_time];
}
...
@property (copy,readonly) QCloudPNTcpPingHandler complete;
...
@implementation QCloudPNTcpPing

- (instancetype)init:(NSString *)host
                port:(NSUInteger)port
               count:(NSUInteger)count
            complete:(QCloudPNTcpPingHandler)complete
{
    if (self = [super init]) {
        _host = host;
        _port = port;
...
- (QCloudPNTcpPingResult *)constPingRes:(NSInteger)code
                               ip:(NSString *)ip
...
{
    if (code != 0 && code != -5) {
        return [[QCloudPNTcpPingResult alloc] init:ip loss:1 count:1 max:0 min:0 avg:0];
    }
    ...
    return [[QCloudPNTcpPingResult alloc] init:ip loss:loss count:count max:max min:min avg:avg];
}
@end
