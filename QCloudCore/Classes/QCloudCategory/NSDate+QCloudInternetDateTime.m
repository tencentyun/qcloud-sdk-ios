//
//  NSDate+QCloudInternetDateTime.m
//  MWFeedParser
//
...
@implementation NSDate (QCloudInternetDateTime)

// Instantiate single date formatter
+ (NSDateFormatter *)qcloud_internetDateTimeFormatter {
    @synchronized(self) {
...
+ (NSDate *)qcloud_dateFromInternetDateTimeString:(NSString *)dateString formatHint:(DateFormatHint)hint {
	...
	NSDate *date = nil;
    if (dateString) {
        if (hint != QCloudDateFormatHintRFC3339) {
            // Try RFC822 first
            date = [NSDate qcloud_dateFromRFC822String:dateString];
            if (!date) date = [NSDate qcloud_dateFromRFC3339String:dateString];
        } else {
            // Try RFC3339 first
            date = [NSDate qcloud_dateFromRFC3339String:dateString];
            if (!date) date = [NSDate qcloud_dateFromRFC822String:dateString];
        }
    }
...
+ (NSDate *)qcloud_dateFromRFC822String:(NSString *)dateString {
    ...
    NSDate *date = nil;
    if (dateString) {
        NSDateFormatter *dateFormatter = [NSDate qcloud_internetDateTimeFormatter];
