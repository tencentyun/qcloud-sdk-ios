...
//    sourceTestBucket =  [[QCloudCOSXMLTestUtility sharedInstance] createTestBucketWithPrefix:@"csp"];
...
//    put.bucket = sourceTestBucket.name;
...
//     get.maxKeys = 1000;
//     get.bucket = sourceTestBucket.name;
//
...
//    putCORS.bucket = sourceTestBucket.name;
...
//                corsReqeust.bucket = sourceTestBucket.name;
...
//                    delCors.bucket = sourceTestBucket.name;
...
//        headerRequest.bucket = sourceTestBucket.name;
...
//        deleteObjectRequest.bucket = sourceTestBucket.name;
...
//    request.bucket = sourceTestBucket.name;
...
//    }];
//
//    [put setInitMultipleUploadFinishBlock:^(QCloudInitiateMultipartUploadResult *multipleUploadInitResult, QCloudCOSXMLUploadObjectResumeData
//    resumeData) {
...
//    [put setFinishBlock:^(id outputObject, NSError *error) {
//        NSURL* serviceURL = [[self getCSPCOSXMLService].configuration.endpoint serverURLWithBucket:sourceTestBucket.name appID:kAppIDCSP
//        regionName:put.regionName]; NSMutableString* objectCopySource = [serviceURL.absoluteString mutableCopy] ; [objectCopySource
//        appendFormat:@"/%@",copyObjectSourceName]; objectCopySource = [[objectCopySource substringFromIndex:7] mutableCopy];
...
//    initrequest.bucket = sourceTestBucket.name;
...
//    }];
//    NSString* expectedBucketString = [NSString stringWithFormat:@"%@",sourceTestBucket.name];
