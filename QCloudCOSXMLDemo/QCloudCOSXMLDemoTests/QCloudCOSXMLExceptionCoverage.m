...
#define TEST_BUCKET @"test-12345678"
...
- (void)testCommonPrefix {
    ...
    XCTAssert([transOutput[@"Prefix"] isEqualToString:@"afsd"]);
   ...
   NSDictionary* dicOutput = [prefix performSelector:@selector(modelCustomWillTransformFromDictionary:) withObject:dictInput];
    XCTAssert([dicOutput[@"Prefix"] isKindOfClass:[NSDictionary class]]);
    ...
    NSDictionary* dictOutput2 = [prefix performSelector:@selector(modelCustomWillTransformFromDictionary:) withObject:dictInput2];
     XCTAssert([dictOutput2[@"Prefix"] isKindOfClass:[NSDictionary class]]);
...
- (void)testQCloudTextRecognitionResult {
    QCloudLogInfo(@"QCloudCOSXMLExceptionCoverage:QCloudTextRecognitionResult");
    QCloudTextRecognitionResult *abortUpload = [[QCloudTextRecognitionResult alloc] init];
    XCTAssert([QCloudLifecycleAbortIncompleteMultipartUpload performSelector:@selector(modelCustomPropertyMapper)]);
    XCTAssert([abortUpload performSelector:@selector(modelCustomWillTransformFromDictionary:) withObject:[NSDictionary dictionary]]);
    XCTAssert([abortUpload performSelector:@selector(modelCustomWillTransformFromDictionary:) withObject:nil] == nil);
}
...
- (void)testCoverGetObjectRequest1 {
    QCloudLogInfo(@"QCloudCOSXMLExceptionCoverage:testCoverGetObjectRequest1");
    QCloudGetObjectRequest *request = [[QCloudGetObjectRequest alloc] init];
    request.bucket = @"testBucket";
    ...
    error = nil;
    request.object = @"a/b/c/d.jpg";
    [request buildRequestData:&error];
    XCTAssertNil(error);
    
    error = nil;
    request.object = @"a/b/c/d.";
    [request buildRequestData:&error];
    XCTAssertNil(error);
    
    error = nil;
    request.object = @"a/b/c/";
    [request buildRequestData:&error];
    XCTAssertNil(error);
...
-(void)testQCloudGetDiscernMediaJobsRequestEx{
    ...
    request.startCreationTime = @"2024-01-01";
    request.endCreationTime = @"2024-01-01";
