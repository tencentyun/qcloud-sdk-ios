...
- (void)viewWillLayoutSubviews {
    ...
    _progressView.frame = CGRectMake(margin, _imgPreviewView.frame.origin.y + _imgPreviewView.frame.size.height + margin * 3,
                                     SCREEN_WIDTH - 120 - margin * 3, height);

    _labUploadState.frame = CGRectMake(_progressView.frame.size.width + _progressView.frame.origin.x + margin, 0,
                                       SCREEN_WIDTH - _progressView.frame.size.width - 3 * margin, height);
    ...
    _sgcSetAcl.frame = CGRectMake(_labAclTitle.frame.origin.x + _labAclTitle.frame.size.width + margin, _labAclTitle.frame.origin.y,
                                  SCREEN_WIDTH - (_labAclTitle.frame.origin.x + _labAclTitle.frame.size.width + margin * 2), height);

    _btnStartUpload.frame = CGRectMake(margin, _labAclTitle.frame.origin.y + _labAclTitle.frame.size.height + margin * 3, 80, height);

    _btnPauseOrGoonUpload.frame
        = CGRectMake(margin + _btnStartUpload.frame.size.width + _btnStartUpload.frame.origin.x, _btnStartUpload.frame.origin.y, 80, height);

    _btnCancelUpload.frame = CGRectMake(margin + _btnPauseOrGoonUpload.frame.size.width + _btnPauseOrGoonUpload.frame.origin.x,
                                        _btnStartUpload.frame.origin.y, 80, height);

    _tvResult.frame
        = CGRectMake(margin, _btnCancelUpload.frame.origin.y + _btnCancelUpload.frame.size.height + margin * 2, SCREEN_WIDTH - margin * 2, 300);
}
...
//    put.object = @"1.png";
//
//    put.bucket = @"ci-1253653367";
//    put.regionName = @"ap-guangzhou";
//
//    put.body = [NSURL fileURLWithPath:self.filePath];
//
