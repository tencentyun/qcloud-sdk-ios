    ...
    _request.finishBlock = ^(id outputObject, NSError *error) {
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (error) {
                ...
                weakself.labDownloadSpeed.text =
                    [NSString stringWithFormat:@"平均下载速度：%.2f %@/s", fileUrl.fileSizeSmallerThan1024 / timeSpent, fileUrl.fileSizeCount];
            }
        });
    };
