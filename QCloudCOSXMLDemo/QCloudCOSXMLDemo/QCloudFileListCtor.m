...
- (void)setupUI {
    ...
    UIBarButtonItem *rightItem = [[UIBarButtonItem alloc] initWithTitle:@"上传"
...
- (void)fetchData {
    ...
    //                  delimiter:路径分隔符 固定为 /
    //                  prefix:路径起始位置，比如要获取bucket下所有文件以及文件夹 改参数就不传，如果要获取folder文件夹下的所有文件及文件夹 该参数就传
    //                  /folder1
    //        假如一个桶的目录如下
    //            |-bucket
    //              |-folder1
    ...
    //                     commonPrefixes 包含该路径下的所有文件夹， prefix代表当每一个文件夹的路径，如果需要获取里面所有文件，需要将该参数传给
    //                     QCloudGetBucketRequest 对象的 prefix
    QCloudGetBucketRequest *request = [QCloudGetBucketRequest new];
//    request.networkType = QCloudRequestNetworkQuic;
//    request.endpoint = [[QCloudCOSXMLEndPoint alloc]initWithLiteralURL:[NSURL URLWithString:@"https://www.eo.test.com"]];
//    request.endpoint.useHTTPS = YES;
...
- (void)cellDeleteFile:(QCloudBucketContents *)content {
    ...
    DEF_WeakSelf(self)[deleteRequest setFinishBlock:^(id _Nullable outputObject, NSError *_Nullable error) {
        dispatch_async(dispatch_get_main_queue(), ^{
            [weakself fetchData];
        });
    }];
...
//    QCloudGetDescribeMediaBucketsRequest * reqeust1 = [[QCloudGetDescribeMediaBucketsRequest alloc]init];
//    reqeust1.regionName = CURRENT_REGION;
//    reqeust1.pageSize = 100;
//    reqeust1.finishBlock = ^(QCloudDescribeMediaInfo * outputObject, NSError *error) {
//        [outputObject.MediaBucketList enumerateObjectsUsingBlock:^(QCloudDescribeMediaBucketItem * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
//            NSLog(@"%@",obj.Name);
//        }];
//    };
//
//    [CURRENT_SERVICE CIGetDescribeMediaBuckets:reqeust1];
    
    
//    QCloudGetMediaInfoRequest * reqeust = [[QCloudGetMediaInfoRequest alloc]init];
//    reqeust.bucket = CURRENT_BUCKET;
//    reqeust.object = @"test.mp4";
//    reqeust.finishBlock = ^(id outputObject, NSError *error) {
//        NSLog(@"123");
//    };
//    [CURRENT_SERVICE CIGetMediaInfo:reqeust];
...
//    shotRequest.finishBlock = ^(QCloudGenerateSnapshotResult * outputObject, NSError *error) {
//
//        NSLog(@"123");
//
//    };
//    [CURRENT_SERVICE GetGenerateSnapshot:shotRequest];
