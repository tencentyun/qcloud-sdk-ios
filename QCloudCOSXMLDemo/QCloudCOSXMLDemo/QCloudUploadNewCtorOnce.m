...
@interface QCloudUploadNewCtorOnce () <UIImagePickerControllerDelegate, UINavigationControllerDelegate>
...
@implementation QCloudUploadNewCtorOnce
...
- (void)viewDidLoad {
    ...
    // 注册 cos 服务实例，无需实现 QCloudSignatureProvider & QCloudCredentailFenceQueueDelegate;
...
- (void)setupUI {
    ...
    _labTip.text = @"体验之前，请在 QCloudUploadNewCtorOnce 类中的 setCOSFileInfo:方法内配置单次临时密钥以及文件信息";
...
-(void)setupCOSXMLService{
    ...
    [QCloudCOSXMLService registerCOSXMLWithConfiguration:configuration withKey:@"OnceCredential"];
    [QCloudCOSTransferMangerService registerCOSTransferMangerWithConfiguration:configuration withKey:@"OnceCredential"];
}
...
- (void)advancedUpload:(QCloudCOSXMLUploadObjectRequest *)upload {
    upload.credential = self.credential;
    ...
    [[QCloudCOSTransferMangerService costransfermangerServiceForKey:@"OnceCredential"] UploadObject:upload];
}
