...
- (void)setupUI {
    ...
    UIBarButtonItem *rightItem = [[UIBarButtonItem alloc] initWithTitle:@"新建桶"
                                                                  style:UIBarButtonItemStylePlain
                                                                 target:self
                                                                 action:@selector(createBucket)];
...
- (void)fetchData {
    ...
    DEF_WeakSelf(self)[request setFinishBlock:^(QCloudListAllMyBucketsResult *_Nullable result, NSError *_Nullable error) {
        DEF_StrongSelf(self)
            self.contentsArray
            = [result.buckets mutableCopy];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    configuration.endpoint.useHTTPS = YES;
...
//    configuration.networkStrategy = QCloudRequestNetworkStrategyAggressive;
    ...
    if (![QCloudCOSXMLService hasCosxmlServiceForKey:regionName]) {
