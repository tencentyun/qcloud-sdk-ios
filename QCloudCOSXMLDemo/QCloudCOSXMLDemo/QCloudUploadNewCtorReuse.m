...
@interface QCloudUploadNewCtorReuse () <UIImagePickerControllerDelegate, UINavigationControllerDelegate,QCloudSignatureProvider,QCloudCredentailFenceQueueDelegate>
...
@implementation QCloudUploadNewCtorReuse
...
- (void)viewDidLoad {
    ...
    // 注册 cos 服务实例。需实现 QCloudSignatureProvider & QCloudCredentailFenceQueueDelegate;
    [self setupCOSXMLService];
}

-(void)setCOSFileInfo{
    self.appID = @"1253960454";
    self.bucket = @"jssdktest-1253960454";
...
- (void)setupUI {
    ...
    _labTip.text = @"体验之前，请在 QCloudUploadNewCtorReuse 类中的 setCOSFileInfo:文件信息，fenceQueue:配置请求临时密钥";
...
-(void)setupCOSXMLService{
    ...
    [QCloudCOSXMLService registerCOSXMLWithConfiguration:configuration withKey:@"ReuseCredential"];
    [QCloudCOSTransferMangerService registerCOSTransferMangerWithConfiguration:configuration withKey:@"ReuseCredential"];
}
...
- (void)advancedUpload:(QCloudCOSXMLUploadObjectRequest *)upload {
    ...
    [[QCloudCOSTransferMangerService costransfermangerServiceForKey:@"ReuseCredential"] UploadObject:upload];
}
