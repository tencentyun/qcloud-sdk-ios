...
@implementation QCloudCIImageRepairRequest
- (void) dealloc
{
}
-  (instancetype) init
...
- (BOOL) buildRequestData:(NSError *__autoreleasing *)error
{
    ...
    [self.requestData setQueryStringParamter:@"ImageRepair" withKey:@"ci-process"];
    
    if(self.maskPic){
        NSData *data = [self.maskPic dataUsingEncoding:NSUTF8StringEncoding];
        NSString *base64ImageUrl = [data base64EncodedStringWithOptions:0];
        [self.requestData setQueryStringParamter:base64ImageUrl withKey:@"MaskPic"];
    }
    
    
    if(self.maskPoly){
        NSData *data = [[self.maskPoly qcloud_modelToJSONString] dataUsingEncoding:NSUTF8StringEncoding];
        NSString *base64ImageUrl = [data base64EncodedStringWithOptions:0];
        [self.requestData setQueryStringParamter:base64ImageUrl withKey:@"MaskPoly"];
    }
