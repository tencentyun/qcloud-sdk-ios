...
- (BOOL) buildRequestData:(NSError *__autoreleasing *)error
{
    ...
    if (!self.selectType || ([self.selectType isKindOfClass:NSString.class] && ((NSString*)self.selectType).length == 0)) {
        if (error != NULL) {
            *error = [NSError qcloud_errorWithCode:QCloudNetworkErrorCodeParamterInvalid message:[NSString stringWithFormat:@"paramter[selectType] is invalid (nil), it must have some value. please check it"]];
            return NO;
        }
    }
