...
- (BOOL) buildRequestData:(NSError *__autoreleasing *)error
{
    ...
    if (!self.origin || ([self.origin isKindOfClass:NSString.class] && ((NSString*)self.origin).length == 0)) {
        if (error != NULL) {
            *error = [NSError qcloud_errorWithCode:QCloudNetworkErrorCodeParamterInvalid message:[NSString stringWithFormat:@"InvalidArgument:paramter[origin] is invalid (nil), it must have some value. please check it"]];
            return NO;
        }
    }
    if (self.origin) {
    ...
    if (!self.accessControlRequestMethod || ([self.accessControlRequestMethod isKindOfClass:NSString.class] && ((NSString*)self.accessControlRequestMethod).length == 0)) {
        if (error != NULL) {
            *error = [NSError qcloud_errorWithCode:QCloudNetworkErrorCodeParamterInvalid message:[NSString stringWithFormat:@"InvalidArgument:paramter[accessControlRequestMethod] is invalid (nil), it must have some value. please check it"]];
            return NO;
        }
    }
...
-(NSArray<NSMutableDictionary *> *)scopesArray{
    ...
    dic[@"action"] = @"name/cos:OptionsObject";
