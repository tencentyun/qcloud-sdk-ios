...
@implementation QCloudGetBucketInventoryRequest
- (void) dealloc
{
}
-  (instancetype) init
...
- (void) configureReuqestSerializer:(QCloudRequestSerializer *)requestSerializer  responseSerializer:(QCloudResponseSerializer *)responseSerializer
{
    ...
    NSArray* responseSerializers = @[
                                    ...
                                    QCloudResponseObjectSerilizerBlock([QCloudInventoryConfiguration class])
                                    ];
...
- (BOOL) buildRequestData:(NSError *__autoreleasing *)error
{
    ...
    if (!self.inventoryID || ([self.inventoryID isKindOfClass:NSString.class] && ((NSString*)self.inventoryID).length == 0)) {
        if (error != NULL) {
            *error = [NSError qcloud_errorWithCode:QCloudNetworkErrorCodeParamterInvalid message:[NSString stringWithFormat:@"paramter[inventoryID] is invalid (nil), it must have some value. please check it"]];
            return NO;
        }
    }
