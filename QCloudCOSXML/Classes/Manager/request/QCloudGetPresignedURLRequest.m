...
- (NSURLRequest*) buildURLRequest:(NSError *__autoreleasing *)error {
    ...
    NSError* buildError;
    if (!self.bucket) {
        buildError =[NSError qcloud_errorWithCode:QCloudNetworkErrorCodeParamterInvalid message:[NSString stringWithFormat:@"InvalidArgument:paramter[bucket] which cannot be nil is invalid (nil)"]];
    }
    NSMutableString* URLString = [[NSMutableString alloc] init];
    [URLString appendString:[self.runOnService.configuration.endpoint serverURLWithBucket:self.bucket appID:self.runOnService.configuration.appID regionName:self.regionName].absoluteString];
    if (self.object) {
