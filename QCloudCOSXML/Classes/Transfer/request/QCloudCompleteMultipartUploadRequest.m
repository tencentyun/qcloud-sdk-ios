...
- (BOOL) buildRequestData:(NSError *__autoreleasing *)error
{
    ...
    if (!self.parts) {
        if (error != NULL) {
            *error = [NSError qcloud_errorWithCode:QCloudNetworkErrorCodeParamterInvalid message:[NSString stringWithFormat:@"paramter[parts] is invalid (nil), it must have some value. please check it"]];
            return NO;
        }
    }
