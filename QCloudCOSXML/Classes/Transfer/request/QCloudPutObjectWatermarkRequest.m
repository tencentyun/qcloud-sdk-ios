...
@implementation QCloudPutObjectWatermarkRequest
- (void) dealloc
{
}
-  (instancetype) init
...
- (BOOL) buildRequestData:(NSError *__autoreleasing *)error
{
    ...
    if (self.picOperations == nil || [self.picOperations getPicOperationsJson] == nil) {
        if (error != NULL) {
            *error = [NSError qcloud_errorWithCode:QCloudNetworkErrorCodeParamterInvalid message:[NSString stringWithFormat:@"InvalidArgument:paramter[Pic-Operations] is invalid (nil), it must have some value. please check it"]];
            return NO;
        }
    }
